...
def set_background(viewbox, colour):
    # set a path of colour the size of the viewbox
    return f"<path id='background' d='M{viewbox[0]} {viewbox[1]} " \
        + f"L{viewbox[2]-viewbox[0]} {viewbox[3]-viewbox[1]} " \
        + f"' stroke='none' fill='{colour}' />"


utils.print_params(DEFAULT)

svg_list = []
svg_list.append(set_background(DEFAULT['DRAWABLE_AREA'], "#000"))
